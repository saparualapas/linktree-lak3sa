...
const MAX_PHOTO_B64  = 420000; // ~300 KB file → ~410 KB base64
...
function setupSheets() {
  ...
  ensureSheet(SHEET_FLOATERS, ['url','size','left','top','created_at']);
...
function getAll() {
  // Profile
  const pr = sh(SHEET_PROFILE).getRange(2,1,1,6).getValues()[0];
  const profile = {
    name:       String(pr[0]||''),
    tagline:    String(pr[1]||''),
    avatar_url: String(pr[2]||''),
    color1:     String(pr[3]||'#2979d4'),
    color2:     String(pr[4]||'#1a4a9e'),
  };
  ...
  for (let i=1; i<fd.length; i++) {
    const r=fd[i]; if(!r[0]) continue;
    floaters.push({ url:String(r[0]||''), size:Number(r[1]||120), left:Number(r[2]||10), top:Number(r[3]||20), _row:i+1 });
  }

  // Photos — pastikan sheet ada
  const photos = [];
...
function saveProfile(body) {
  sh(SHEET_PROFILE).getRange(2,1,1,6).setValues([[
...
function addFloater(body) {
  ...
  sh(SHEET_FLOATERS).appendRow([
    sanitize(body.url,300), Number(body.size)||120,
    Number(body.left)||10, Number(body.top)||20, new Date().toISOString(),
  ]);
  return { ok:true };
}
...
function updateFloater(body) {
  const row = validRow(body.rowIndex);
  if (!body.url) throw new Error('URL wajib diisi');
  sh(SHEET_FLOATERS).getRange(row,1,1,5).setValues([[
    sanitize(body.url,300), Number(body.size)||120,
    Number(body.left)||0, Number(body.top)||0, new Date().toISOString(),
  ]]);
  return { ok:true };
}
...
function addPhoto(body) {
  ...
  if (body.data_b64.length > MAX_PHOTO_B64) {
    throw new Error('Foto terlalu besar. Maksimal 300 KB.');
  }
